...
namespace PptCOM
{
    ...
    public class PptCOMServer : IPptCOMServer
    {
        ...
        private unsafe void FullCleanup(bool final = false)
        {
            ...
            try
            {
                if (pptTotalPage != null) *pptTotalPage = -1;
                if (pptCurrentPage != null) *pptCurrentPage = -1;
            }
            catch { }
        ...
        [ComVisible(false)]
        public unsafe bool EnsureBoundToActivePowerPoint()
        {
            int bestPriority = 0;
            int targetPriority = 0;
            object bestApp = null;
            ...
            try
            {
                bestApp = GetAnyActivePowerPoint(pptApplication, out bestPriority, out targetPriority);
            }
            catch
            {
                return false;
            ...
            if (bestApp == null)
            {
                try
                {
                    if (pptApplication != null) FullCleanup();
                }
                catch { }
                return false;
            }

            bool needRebind = pptApplication == null || !AreComObjectsEqual((object)pptApplication, bestApp);
            if (needRebind)
            {
                try
                {
        ...
        [ComVisible(false)]
        public bool TryGetSlideShowPageIndexTotal(out int currentPage, out int totalPage)
        {
            currentPage = -1;
            totalPage = -1;

            if (!EnsureBoundToActivePowerPoint()) return false;

            try
            {
                totalPage = GetTotalSlideIndex(pptActivePresentation);
            }
            catch
            {
                totalPage = -1;
            ...
            try
            {
                currentPage = GetCurrentSlideIndex(pptSlideShowWindow);
            }
            catch
            {
                currentPage = -1;
