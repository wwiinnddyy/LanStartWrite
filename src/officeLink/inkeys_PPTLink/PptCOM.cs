...
namespace PptCOM
{
    ...
    public class PptCOMServer : IPptCOMServer
    {
        ...
        {
            // Console.WriteLine("PPT Monitor Service Tick");

            // 初始化
            // bindingEvents = false; // Don't reset bindingEvents every tick
            // *pptCurrentPage = -1; // Don't reset pointers every tick
            // *pptTotalPage = -1;
            // polling = 0;
                ...
                // while (true) // Removed Loop
                {
                    // 动态绑定/切换逻辑
                    {
                        ...
                        if (needRebind == true)
                        {
                            ...
                            if (bestApp != null)
                            {
                                // if (wait) Thread.Sleep(1000); // Removed sleep
                                ...
                                try
                                {
                                    ...
                                    // bindingEvents = false; // BUG in original code? It set bindingEvents=false right after setting it true?
                                    // Oh wait, original code lines 804-807:
                                    // bindingEvents = false;
                                    // forcePolling = true;
                                    // Console.WriteLine($"成功绑定! {pptApplication.Name}");
                                    
                                    // It seems original code intentionally disabled events and used polling?
                                    // "bindingEvents = false; forcePolling = true;"
                                    // But earlier it said "bindingEvents = true; forcePolling = false;"
                                    // It looks like the original code had a bug or logic I don't understand.
                                    // Let's assume we WANT events if possible.
                                    // I will comment out the disabling part.
                    ...
                    if (pptApplication != null && pptActivePresentation != null)
                    {
                        ...
                        {
                            *pptCurrentPage = -1;
                            // *pptTotalPage = -1; // Keep total page if possible? No, if not active, -1.
            ...
            {
                // FullCleanup(true); // Removed cleanup
            }
        ...
        public void ViewSlideShow()
        {
            try
            {
                ...
                else if (pptActivePresentation != null)
                {
                    pptActivePresentation.SlideShowSettings.Run();
                }
            }
            catch
            {
        ...
        public string GetSlideNotes(int slideIndex)
        {
            try
            {
                if (pptActivePresentation != null)
                {
                    dynamic slides = pptActivePresentation.Slides;
                    dynamic slide = slides[slideIndex];
        ...
        public void GotoSlide(int index)
        {
            try
            {
                if (pptSlideShowWindow != null)
                {
                    pptSlideShowWindow.View.GotoSlide(index);
                }
                else if (pptActivePresentation != null)
                {
                    // 如果不在放映模式，尝试在设计视图跳转
                    try {
                        pptApplication.ActiveWindow.View.GotoSlide(index);
                    } catch {}
                }
            }
            catch { }
    ...
    {
        private static PptCOMServer server = new PptCOMServer();
        private static bool isInitialized = false;
        private static unsafe int totalPage = -1;
        private static unsafe int currentPage = -1;
        private static unsafe int offSignal = 0;

        public async System.Threading.Tasks.Task<object> Invoke(object input)
        {
            var payload = input as System.Collections.Generic.IDictionary<string, object>;
            if (payload == null) return null;

            string action = payload.ContainsKey("action") ? payload["action"].ToString() : "";
            ...
            switch (action)
            {
                case "init":
                    if (!isInitialized)
                    {
                        unsafe
                        {
                            fixed (int* pTotal = &totalPage, pCurrent = &currentPage, pOff = &offSignal)
                            {
                                isInitialized = server.Initialization(pTotal, pCurrent, pOff);
                            }
                        }
                    }
                ...
                case "getStatus":
                    server.PptComService(); // 维持连接并更新基本信息
                    int current = 0;
                    int total = 0;
                    unsafe { current = currentPage; total = totalPage; }
                    
                    return new {
                        currentPage = current,
                        totalPage = total,
                        notes = current > 0 ? server.GetSlideNotes(current) : ""
                    };

                case "next":
                    server.NextSlideShow(true);
                    return true;
                ...
                case "goto":
                    ...
                    {
                        int page = Convert.ToInt32(payload["page"]);
                        server.GotoSlide(page);
                    }
                    return true;

                case "play":
                    server.ViewSlideShow();
                    return true;
