...
import { exportDbToCunoxDir, exportDbToCunoxFile, importCunoxDirToDb, importCunoxFileToDb } from '..'
...
describe('cunox directory format', () => {
  it('exports from db and imports back with same semantics', async () => {
    ...
    const outDir = join(base, 'book-dir.cunox')
  ...
  it('exports and imports as single archive file', async () => {
    const base = await mkdtemp(join(tmpdir(), 'cunox-test-'))
    const dbDir1 = join(base, 'db1')
    const dbDir2 = join(base, 'db2')
    const outFile = join(base, 'book-file.cunox')
    ...
    try {
      ...
      await exportDbToCunoxFile(db1, { outFile, overwrite: true })
      await importCunoxFileToDb(db2, { file: outFile, mode: 'replace' })
